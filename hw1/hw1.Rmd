...
**For Q3.5, I used command "awk 'NR > 1 {print}'" to omit the first line for 
printing.**
...
**Here I notice that there may be more than one name in the same line, so I use
the command "grep -o" to print only the matched (non-empty) parts of a matching
line, with each such part on a separate output line. And this method will solve
the "multiple names in the same line" problem.**
...
```{bash}
echo "Display the username of the current user."
who am i
```
