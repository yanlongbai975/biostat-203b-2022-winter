...
if (Sys.info()[["nodename"]] == "biostat-203b-teaching-server") {
  ...
  satoken <- "/Users/a123456/biostat-203b-2022-winter/biostat-203b-2022winter-3fdc2392ac39.json"
}
...
labevents_tble = inner_join(labevents_tble, labcharttm, by = c("subject_id", "itemid", "charttime")) %>%
  pivot_wider(names_prefix = "", names_from = c("itemid"), values_from = c("charttime","valuenum")) %>%
  print(width = Inf)
summarise(labevents_tble, n = n())
rm(lab_mes)
rm(interest_lab_mes)
...
chartevents_tble = inner_join(chartevents_tble, chartevecharttm, by = c("subject_id", "itemid", "charttime")) %>%
  pivot_wider(names_prefix = "", names_from = c("itemid"), values_from = c("charttime","valuenum")) %>%
  print(width = Inf)
summarise(chartevents_tble, n = n())
...
mimic_icu_cohort = left_join(icustays_tble, admissions_tble, by = c("subject_id", "hadm_id")) %>%
  ...
  mutate(hadm_to_death = if(is.na(dod)) Inf else as.character(dod-admittime)) %>%
  mutate(thirty_day_mort = if(str_length(hadm_to_death) != 8){if(as.numeric(substr(hadm_to_death, 5, 6)) <= 30) 1 else 0} else 0) %>%
  print(width = Inf) # %>%
