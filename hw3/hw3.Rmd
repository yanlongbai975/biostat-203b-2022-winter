...
mimic_icu_cohort = left_join(icustays_tble, admissions_tble, 
  ...
  mutate(hadm_to_death = if(is.na(dod)) NA
         else DATE_DIFF(date(dod), date(admittime), DAY)) %>%
  mutate(thirty_day_mort = ifelse(
    is.na(dod), FALSE, DATE_DIFF(date(dod), date(admittime), DAY) <= 30)) %>%
  print(width = Inf)  %>%
  collect() %>%
  #write_rds(
