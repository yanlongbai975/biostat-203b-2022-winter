  ...
  if(!is.timepoint(colpoint[[1]])){
    if(!is.character(colpoint[[1]])){
      if(!is.integer(colpoint[[1]])){
        ...
        #18th column: hospital_expire_flag.
...
**this imputation during**
**knitting and just directly read in the rds file containing this imputed**
**data set.**

```{r, eval = FALSE}
#Runing imputation in parallel.
cl = makeCluster(4)
...
if (Sys.info()[["nodename"]] == "biostat-203b-teaching-server") {
  ...
  "/home/yanlongbai975/biostat-203b-2022-winter/hw4/icu_cohort_imp.rds"
  )
} else {
  # on my own Mac laptop
  icucohort_tble_imp = read_rds(
...
**The red lines in these plots are the density of the original, non-missing data.**
...
**For categorical variables, the distribution of the number of distinct levels**
**imputed for each sample is shown next to the distribution of unique draws from**
**that variable in the nonmissing data, given that the draws were completely random.**
...
plot(model_glmnet)
coef(model_glmnet, s = model_glmnet$lambda)
...
```
```{r}
problogi = predict(model_glm, icu_test, type = "response")
predlogi = ifelse(problogi > 0.5, TRUE, FALSE)
table(observed = icu_test$thirty_day_mort, predicted = predlogi)
cat('Accuracy', mean(predlogi == icu_test$thirty_day_mort))
```
...
```
```{r}
problogi = predict.glmnet(model_glmnet, x_test, type = "response")
predlogi = ifelse(problogi > 0.5, TRUE, FALSE)
predlogi_final = ifelse(rowSums(predlogi) > 30, TRUE, FALSE)

table(observed = icu_test$thirty_day_mort, predicted = predlogi_final)
cat('Accuracy', mean(predlogi == icu_test$thirty_day_mort))
```
